...
const getApiUrl = () => {
  ...
  try {
    if (Capacitor.isNativePlatform()) {
      console.log('📱 Native platform (Android/iOS) tespit edildi - Render.com backend kullanılıyor');
      ...
      const apiUrl = 'https://backend-x49x.onrender.com';
      console.log('✅ Android/iOS için API URL:', apiUrl);
      return apiUrl;
    }
  } catch (error) {
    console.warn('⚠️ Capacitor kontrolü hatası:', error);
  }
...
// Android için timeout daha uzun (network gecikmesi için)
const isNative = Capacitor.isNativePlatform();
const timeout = isNative ? 120000 : 90000; // Android: 120s, Web: 90s
...
api.interceptors.response.use(
  ...
  (error: AxiosError<any>) => {
    if (error.response) {
      ...
      console.error('❌ Network hatası detayları:', {
        code: error.code,
      ...
      if (error.code === 'ECONNREFUSED' || error.code === 'ERR_NETWORK' || error.message?.includes('Network Error')) {
        const message = isNative 
          ? `Backend sunucusuna bağlanılamadı (${apiUrl}). Lütfen internet bağlantınızı kontrol edin.`
          : 'Backend sunucusuna bağlanılamadı. Lütfen backend servisinin çalıştığından emin olun.';
        toast.error(message);
      } else if (error.code === 'ETIMEDOUT' || error.code === 'ECONNABORTED' || error.message?.includes('timeout')) {
        const message = isNative
          ? 'Bağlantı zaman aşımına uğradı. Render.com backend uyanıyor olabilir, lütfen tekrar deneyin.'
          : 'Bağlantı zaman aşımına uğradı. Lütfen internet bağlantınızı kontrol edin.';
        toast.error(message);
      } else {
        const message = isNative
          ? `Sunucuya bağlanılamadı (${apiUrl}). Lütfen internet bağlantınızı kontrol edin.`
...
const retryRequest = async (requestFn: () => Promise<any>, maxRetries = 4, delay = 2000) => {
  const isNative = Capacitor.isNativePlatform();
  // Android için daha fazla retry ve daha uzun delay
  const finalMaxRetries = isNative ? 6 : maxRetries;
  const initialDelay = isNative ? 4000 : delay;
  
  console.log(`🔄 Retry mekanizması başlatıldı: ${finalMaxRetries} deneme, ${initialDelay/1000}s başlangıç delay (${isNative ? 'Android' : 'Web'})`);
  ...
  for (let i = 0; i < finalMaxRetries; i++) {
    try {
      ...
      if (i === finalMaxRetries - 1 || !isNetworkError) {
        console.error(`❌ API isteği başarısız (${i + 1}/${finalMaxRetries}):`, {
          code: error.code,
          message: error.message,
      ...
      const maxDelay = isNative ? 15000 : 10000;
      const waitTime = Math.min(currentDelay, maxDelay);
      
      console.log(`🔄 Backend uyanıyor (${isNative ? 'Android' : 'Web'}), ${waitTime/1000} saniye sonra tekrar deneniyor... (${i + 1}/${finalMaxRetries})`);
